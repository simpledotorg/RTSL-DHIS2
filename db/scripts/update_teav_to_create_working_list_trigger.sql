        ...
        BEGIN
            ...
            VALUES (
                       ...
                       (SELECT trackedentityattributeid
                        ...
                        WHERE uid = 'rgeuEnAI0nj'),
                       'OVERDUE_PENDING_CALL',
                       (SELECT created
                        FROM programinstance
                        WHERE programinstanceid = NEW.programinstanceid),
                       current_timestamp, -- Use current date and time
                       'trigger'
                   );
        EXCEPTION WHEN unique_violation THEN
            UPDATE trackedentityattributevalue
            SET value = 'OVERDUE_PENDING_CALL',
                lastupdated = current_timestamp, -- Use current date and time
                storedby = 'trigger'
            WHERE
                    trackedentityinstanceid = (SELECT trackedentityinstanceid
                                               FROM programinstance
                                               WHERE programinstanceid = NEW.programinstanceid)
              AND trackedentityattributeid = (SELECT trackedentityattributeid
                                              FROM trackedentityattribute
                                              WHERE uid = 'rgeuEnAI0nj');
        END;
    END IF;
...
BEGIN
    ...
    IF NEW.programstageid = (SELECT programstageid FROM programstage WHERE uid = 'anb2cjLx3WM') AND NEW.status = 'SCHEDULE' THEN
...
BEGIN
    -- Check if the newly inserted row has programstageid equal to the given programstage uid
    IF NEW.programstageid = (SELECT programstageid FROM programstage WHERE uid = 'W7BCOaSquMd') THEN
...
DECLARE
    overdue_row RECORD;
    tracked_entity_uid TEXT; -- Variable to store trackedentityinstance UID
BEGIN
    ...
    FOR overdue_row IN
        ...
        LOOP
            ...
            IF current_timestamp + INTERVAL '2 hours' > overdue_row.max_duedate THEN
                ...
                SELECT trackedentityinstanceid INTO tracked_entity_uid
                FROM programinstance
                WHERE programinstanceid = overdue_row.programinstanceid;
