...
BEGIN
    ...
    LOOP
        ...
        IF current_timestamp + INTERVAL '2 hours' > overdue_row.max_duedate THEN
            ...
            SELECT uid INTO tracked_entity_uid
            FROM trackedentityinstance WHERE trackedentityinstanceid = 
            ...
            SELECT EXISTS (
                ...
                WHERE trackedentityinstanceid = (SELECT trackedentityinstanceid 
                    FROM programinstance 
                    WHERE programinstanceid = overdue_row.programinstanceid)
                  AND trackedentityattributeid = (SELECT trackedentityattributeid FROM trackedentityattribute WHERE uid = 'rgeuEnAI0nj')
...
SELECT update_overdue_pending_call_for_existing_data_function();
